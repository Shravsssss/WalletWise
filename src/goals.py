...
def process_set_goal(message, bot):
    ...
    try:
        ...
        if len(parts) != 2:
            raise ValueError("Invalid format. Use: [goal_name] [target_amount].")
        
        goal_name, target_amount = parts[0], float(parts[1])

        # Fetch or initialize user's goals
        goals_collection = get_goals_collection()
        user_goals = goals_collection.find_one({"chatid": str(chat_id)}) or {"chatid": str(chat_id), "goals": {}}

        # Add or update the goal
        user_goals["goals"][goal_name] = {"target": target_amount, "saved": 0.0}
        goals_collection.update_one({"chatid": str(chat_id)}, {"$set": user_goals}, upsert=True)
...
def show_goals_status(message, bot):
    ...
    try:
        ...
        if not user_goals or not user_goals.get("goals"):
            bot.send_message(chat_id, "You have no savings goals set. Use /setGoal to add one.")
            return
...
def process_add_savings(message, bot):
    ...
    try:
        ...
        parts = message.text.split(maxsplit=2)
        if len(parts) != 2:
            raise ValueError("Invalid format. Use: [goal_name] [amount_saved].")
        ...
        if not user_goals or goal_name not in user_goals.get("goals", {}):
            bot.send_message(chat_id, f"Goal '{goal_name}' not found. Use /setGoal to create it.")
            return

        user_goals["goals"][goal_name]["saved"] += amount_saved
        goals_collection.update_one({"chatid": str(chat_id)}, {"$set": user_goals})
