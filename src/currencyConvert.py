...
def create_currency_menu(bot, chat_id, prompt="Select the output currency:"):
    ...
    # Replace with relevant currency list if needed
    currencies = helper.get_currency_options()
    buttons = [types.InlineKeyboardButton(
        c, callback_data=f"currency_{c}") for c in currencies]
    markup.add(*buttons)
...
def handle_currency_selection(bot, call):
    ...
    bot.send_message(
        chat_id,
        f"Output currency set to {currency}. Now, please enter the amount in USD to convert:")
...
def handle_amount_input(bot, message):
    ...
    if chat_id in user_data and user_data[chat_id].get('output_currency'):
        if amount:
            ...
            if converted_amount:
                bot.send_message(
                    chat_id, f"{amount} {input_currency} is {
                        converted_amount:.2f} {output_currency}")
            else:
                bot.send_message(
                    chat_id, "Error retrieving conversion rate. Please try again later.")
        else:
            bot.send_message(
                chat_id, "Invalid amount. Please enter a valid number.")
    else:
